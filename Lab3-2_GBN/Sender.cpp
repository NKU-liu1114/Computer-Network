...
uint16_t Checksum(Datagram d,int flag) {
    ...
    int count = DatagramLen / sizeof(short);// 一个数据报有 count 组
...
bool Shakehand() {
    ...
    bool flag= recvfrom(SenderSocket, (char*)&ReceiveData, DatagramLen, 0, (struct sockaddr*)&Receiver_addr, &len);
...
bool Wavehand() {
    ...
    ReceiveData = { 0 };
    flag = recvfrom(SenderSocket, (char*)&ReceiveData, DatagramLen, 0, (struct sockaddr*)&Receiver_addr, &len);
        ...
        if (Flag_Timer == 1) {
            ...
            while (true) {
                ...
                if (recv != -1 && ReceiveData.acknum >= Base && (check ^ ReceiveData.checksum) == 0xffff) {
                    ...
                    //cout << "右移后，此时窗口大小：" << Base << "---" << NextSeq << endl;
...
int main(){
    ...
    cout << "传输吞吐率(包含协议开销）：" << float(DatagramLen * SeqNumber * 8 / (1000000*DuringTime)) << "Mbps" << endl;
